...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        target_padded = self.vocab.tgt.to_input_tensor(target, device=self.device)   # Tensor: (tgt_len, b)
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device) # (max_sentence_length, b, max_word_length)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens = pad_packed_sequence(sequence=enc_hiddens)[0].transpose(0, 1)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings_target(target_padded.transpose(0, 1)).transpose(0, 1)
        Y_splitted_tuple = torch.split(tensor=Y, split_size_or_sections=1, dim=0)
        for Y_t in Y_splitted_tuple:
